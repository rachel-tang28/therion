...
export default function Pipeline() {
  ...
  const handleSubmit = async (event: any) => {
    ...
    if (activeSteps[4]) {
        try {
            const endpoint = process.env.NEXT_PUBLIC_API_ENDPOINT + 'upload_threshold/';
            console.log("Endpoint: ", endpoint);
            const response = await fetch(endpoint, {
                method: "POST",
                headers: {
    ...
    if (activeSteps[5]) {
        try {
            const endpoint = process.env.NEXT_PUBLIC_API_ENDPOINT + 'upload_allergenicity_threshold/';
            console.log("Endpoint: ", endpoint);
            const response = await fetch(endpoint, {
                method: "POST",
                headers: {
    ...
    if (activeSteps[2]) {
        try {
            const endpoint = process.env.NEXT_PUBLIC_API_ENDPOINT + 'upload_selections/';
            console.log("Endpoint: ", endpoint);
    ...
    if (activeSteps[3]) {
        if (uploadedConservancyFiles.length !== 0) {
            ...
            try {
                const endpoint = process.env.NEXT_PUBLIC_API_ENDPOINT + 'pipeline_name/';
                console.log("Endpoint: ", endpoint);
                const response = await fetch(endpoint, {
                    method: "POST",
                    headers: {
                ...
                <AlertDialogContent>
                    ...
                    <AlertDialogDescription>
                       <p className="text-sm">
                            ...
                            Follow the step-by-step accordion panels to complete your configuration. <br /><br />
                            Steps marked as &quot;<span className="font-bold text-red-700">Required</span>&quot; must be completed before running the pipeline.
                            All other steps have consensus mode enabled by default, which uses all available methods for prediction. <br /><br />
                    ...
                    <AccordionContent>
                        <div className="flex flex-col gap-[0px] first-box">
                            ...
                            <div className={`
                                    ...
                                    overflow-hidden
                                `}>
                                <Toggle className="toggle-button" variant="outline" onClick={() => {
                                    setSelections(prev => prev.includes('netmhcpan_el') ? prev.filter(s => s !== 'netmhcpan_el') : [...prev, 'netmhcpan_el']);
                                }}>
                                    {/* <Image src="/netmhcpan.svg" alt="NetMHCpan Icon" width={16} height={16}></Image> */}
                                    NetMHCpan_el
                                </Toggle>
                                <Toggle className="toggle-button" variant="outline" onClick={() => {
                                    setSelections(prev => prev.includes('netctl') ? prev.filter(s => s !== 'netctl') : [...prev, 'netctl']);
                                }}>
                    ...
                    <AccordionContent>
                        <div className="flex flex-col gap-[0px] first-box">
                            ...
                            <div className="flex w-full flex-col gap-[6px]">
                                ...
                                <div>
                                    <h2 className="parameter-heading">Threshold (must be between -0.5 to 2.0)</h2>
                                    <Input
                                        className="input-text w-1/6 mt-[8px]"
                                        ...
                                        onChange={(e) => {
                                            ...
                                            console.log("Allergenicity threshold value:", e.target.value);
                                        }}
                                    ></Input>
                                </div>
                            </div>
